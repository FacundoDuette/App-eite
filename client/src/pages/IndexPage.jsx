...
export default function IndexPage() {
  ...
  const handleFilter = () => {
    let alojFiltrados = [...copiaAlojamientos]; // Clonamos la lista original para preservar el estado.

    // Aplicamos el filtro de dirección si está definido
    if (filtroDireccion) {
      alojFiltrados = alojFiltrados.filter((alojamiento) =>
        alojamiento.direccion.includes(filtroDireccion)
      );
    }

    // Aplicamos el filtro de precio si está definido
    if (filtroPrecio) {
      alojFiltrados = alojFiltrados.filter(
        (alojamiento) => alojamiento.precioPorNoche <= filtroPrecio
      );
    }

    // Aplicamos el filtro de cantidad de huéspedes si está definido
    if (filtroCantidad) {
      alojFiltrados = alojFiltrados.filter(
        (alojamiento) => alojamiento.cantidadHuespedes >= filtroCantidad
      );
    }

    // Actualizamos el estado solo una vez al final
    setAlojamientos(alojFiltrados.length ? alojFiltrados : copiaAlojamientos);
  };
